...
echo "yes" | keytool -importkeystore -srckeystore $KEYSTORE_DIR/$KEYSTORE -srcstoretype PKCS12 -alias $CERT_ALIAS -destkeystore $PKCS_KEYSTORE_DIR/$PKCS_KEYSTORE -deststoretype PKCS12 -srcstorepass $STOREPASS -deststorepass $STOREPASS -srckeypass $KEYPASS -destkeypass $KEYPASS

openssl pkcs12 -in $PKCS_KEYSTORE_DIR/$PKCS_KEYSTORE -nocerts -out $PEM_KEYSTORE_DIR/$PEM_KEYSTORE -nodes -passin pass:$STOREPASS
